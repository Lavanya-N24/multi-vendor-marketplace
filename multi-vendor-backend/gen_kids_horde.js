...
async function updateDbImagePath(imageName) {
    // We try to match exactly using a regex on 'image' and 'gender' = 'Kids'
    const newPath = `/products/kids/${imageName}`;
    const product = await prisma.product.findFirst({
        where: {
            gender: "Kids",
            title: {
                contains: imageName.replace(".png", "").replace(/[-_]/g, " ").replace("boys ", "Boys ").replace("girls ", "Girls ").replace("baby ", "Baby ")
            }
        }
    });
    ...
    if (product) {
        ...
        // Find whichever product has currently the matching relative file logic
        // We know seed.js seeds it with /products/kids/filename
        const products = await prisma.product.findMany({ where: { gender: "Kids" } });
        for (let p of products) {
            // Check if title words match the file words
...
async function generateOne(item) {
    ...
    for (let retry = 0; retry < 3; retry++) {
        reqRes = await requestImage(item.p);
        if (reqRes.id) break;
        await delay(5000);
    }
    if (!reqRes || !reqRes.id) throw new Error("No job ID: " + JSON.stringify(reqRes));
    ...
    while (attempts < 60) {
        ...
        if (s.done) break;
        attempts++;
        if (attempts % 10 === 0) console.log(`    ... waiting for ${item.name} (${s.wait_time || '?'}s estimated)`);
    }
...
// Process 3 at a time for speed (AI Horde allows concurrency for different workers)
...
async function main() {
    ...
    console.log("Generating " + KIDS.length + " images via AI Horde (just like Men and Women section did)...\n");

    for (let i = 0; i < KIDS.length; i += 3) {
        const batch = KIDS.slice(i, i + 3);
        await processBatch(batch, Math.floor(i / 3), KIDS.length);
        if (i + 3 < KIDS.length) await delay(1000);
    }
